...
impl Assets {
    pub(crate) fn has_ecdsa_key(&self, pk: &DefiniteDescriptorKey) -> bool {
        println!("Assets::has_ecdsa_key()");
        self.keys.iter().any(|(keysource, can_sign)| {
            println!("--");
            println!("Assets::has_ecdsa_key() {}", can_sign.ecdsa);
            println!("Assets::has_ecdsa_key() {}", pk.master_fingerprint() == keysource.0);
            println!("Assets::has_ecdsa_key() {}", is_key_direct_child_of(pk, &keysource.1));
            can_sign.ecdsa
